...
useEffect(() => {
    ...
    async function tick() {
      try {
        ...
        if (r1.status === "fulfilled") {
          const resp = r1.value;
          setN8n({ state: mapStatus(resp.ok, resp.status), status: resp.status });
        } else {
        ...
        if (r2.status === "fulfilled") {
          const resp = r2.value;
          setPy({ state: mapStatus(resp.ok, resp.status), status: resp.status });
        } else {
  ...
  useEffect(() => {
    if (n8n.state === "ok" && py.state === "ok" && typeof onReady === "function") {
      onReady();
    }
    ...
    return (
      <span
        style={{
          ...
          background:color,
          boxShadow: state==="waiting" ? "0 0 0 4px rgba(245,158,11,.2)" : "none"
        }}
      />
    );
  };
  ...
  const Label = ({ state, status }) => {
    if (state === "ok") return <small style={{opacity:.7}}>OK</small>;
    if (state === "waiting") {
      return <small style={{opacity:.7}}>en attente
      {/* {status ? ` (${status})` : ""} */}
      </small>;
    }
    return <small style={{opacity:.7}}>erreur</small>;
  };
  ...
  return (
    <div style={{minHeight:'100dvh', display:'grid', placeItems:'center', top: 0}}>
      <div style={{textAlign:'center', marginTop: -33}}>
      <CoffeeLoader level={level}
  ...
  >
    {dots}
        ...
        <ul style={{listStyle:"none", padding:0, margin:0, display:"grid", gap:8}}>
          <li style={{display:"flex", gap:10, alignItems:"center", justifyContent:"center"}}>
            <Dot state={n8n.state} />
            <span>n8n API</span>
